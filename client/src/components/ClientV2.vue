<template>
    <div>
        <div class="user-card" v-if="isUserValid(props.user)">
            ...
            <div class="user-field">
                <label for="username" class="font-semibold w-24">Job</label>
                <InputText id="username" class="flex-auto" autocomplete="off" :placeholder="props.user.job"/>
            </div>
...
function isUserValid(user) {
    return (user.firstname && user.lastname && user.birthdateday && user.birthdatemonth && user.birthdateyear && user.job);
}
